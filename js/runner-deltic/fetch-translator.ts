...
// stdout (the `verify-deltic` recipe captures it).
//
// THE PIN lives here (TAG + TRANSLATOR_SHA256) and in ../runner-deltic
// import-map URLs (deno.json). `assertPinConsistency` fails loud if the two
// drift. Bumping: update TAG here and in deno.json, update TRANSLATOR_SHA256
// from the release's SHA256SUMS, delete deno.lock, and re-run
// `deno cache runner.ts fetch-translator.ts` in this directory to
// regenerate it (commit the diff).

const TAG = "pre-58875e8";
