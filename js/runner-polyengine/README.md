# js/runner-polyengine

Host-side JS runner for `polymorph:test` suites on Deno via
[polyengine](https://github.com/polymorph-components/polyengine) — a runtime linker, so unlike
...
Deno). Same runner-is-provider topology as `js/runner-node`: polyengine's
...
deno run --allow-read=target --config js/runner-polyengine/deno.json --frozen \
    js/runner-polyengine/runner.ts target/wasm32-wasip2/release/sample_suite.wasm \
    [--jsonl]
```

No fetch step and no extra permissions: the translator arrives with the
pinned `@polyengine/translator` package and loads through the module graph
...
L4 results JSONL (`just verify-polyengine` diffs it, normalized, against
`expected/verify-polyengine-*.jsonl` and folds it through `component-test
fold` against the shared sample fold golden). Tag scheduling comes from
the suite's own embedded inventory (polyengine#25): the fixture leg runs
...
as `verify-polyengine`'s last leg.

Suites that import a SUT host module (`polymorph:websocket`,
`polymorph:webcrypto`, …) use `worker-main.mjs` (exported as
`./polyengine-worker-main`) instead of the stock worker: the downstream
repo bundles one worker entry — the polyengine engine surface, the message
...
polyengine is consumed from JSR as **exact-pinned releases**:
...
bump is one diff. Between releases every green polyengine `main` commit
...
- No sha256 bookkeeping and no release-asset downloads: the browser-leg
  assets are built from the same locked graph by `just polyengine-assets`
  into `target/polyengine-browser/` — `polyengine-embedder.mjs` bundled from
  `browser-bundle-entry.ts`, and `polyengine-translator-shim.wasm` copied
  out of the lock-pinned module cache (the packaged
  `@polyengine/translator` asset). The directory is version-free: the lock
  owns versioning.
- `just polyengine-pin-gate` (a `verify-polyengine` prerequisite, so it runs in
  CI) asserts one version across every `@polyengine` specifier in every
...
gate asserts agreement. Regenerate the `expected/verify-polyengine-*`
